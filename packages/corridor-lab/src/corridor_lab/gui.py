...
class CorridorLabApp:
    """The Corridor Lab main window.

    Tkinter is imported lazily by :func:`launch_gui` and injected here, so this
    class can be constructed against a real Tk root in a test without the module
    importing Tkinter at import time.
    """

    def __init__(self, root: object, ttk: object, scrolledtext: object, filedialog: object, messagebox: object, tk: object) -> None:
        self.root = root
        self._tk = tk
        self._ttk = ttk
        ...
        self.format_var = self._tk.StringVar(value="markdown")
        self.scenario_var = self._tk.StringVar(value=self.controller.scenario_source)
        self.routes_var = self._tk.StringVar(value=self.controller.routes_source)
        self.parameter_var = self._tk.StringVar(value="fx_spread_bps")
        self.values_var = self._tk.StringVar(value="10,25,50,100")
        self.parameter_b_var = self._tk.StringVar(value="fx_rate")
        self.values_b_var = self._tk.StringVar(value="1.7,1.8")
        self.status_var = self._tk.StringVar(value="Load the built-in fictional demo or select your own fictional inputs.")
    ...
    def _build(self) -> None:
        ...
        scrolledtext_module = self._scrolledtext
        ...
        ttk_module.Label(frame, text="Route JSON or folder").grid(row=5, column=0, sticky="w", pady=2)
        ttk_module.Entry(frame, textvariable=self.routes_var, state="readonly").grid(row=5, column=1, sticky="ew", padx=6)
        route_actions = ttk_module.Frame(frame)
        route_actions.grid(row=5, column=2, sticky="e")
        ...
        templates = ttk_module.LabelFrame(frame, text="Synthetic templates", padding=8)
        templates.grid(row=4, column=0, columnspan=3, sticky="ew", pady=(6, 0))
        templates.columnconfigure(1, weight=1)
        ttk_module.Label(templates, text="Template").grid(row=0, column=0, sticky="w", padx=(0, 4))
        self.template_var = self._tk.StringVar()
        self.template_box = ttk_module.Combobox(
            templates,
            textvariable=self.template_var,
            values=list(self.controller.template_ids("scenario")),
            state="readonly",
            width=42,
        )
        self.template_box.grid(row=0, column=1, sticky="ew", padx=(0, 6))
        ttk_module.Button(templates, text="Load into Editor", command=self._load_template).grid(row=0, column=2, padx=(0, 6))
        ttk_module.Button(templates, text="Save Template As...", command=self._save_template).grid(row=0, column=3, padx=(0, 6))
        ttk_module.Button(
            templates,
            text="Save Revision...",
            command=self._save_revision,
        ).grid(row=0, column=4, padx=(0, 6))
        ttk_module.Button(templates, text="Compare Two Scenarios...", command=self._compare_scenarios).grid(row=0, column=5)
        self.template_var.set(self.controller.template_ids("scenario")[0])

        actions = ttk_module.LabelFrame(frame, text="Actions", padding=8)
        actions.grid(row=6, column=0, columnspan=3, sticky="ew", pady=(10, 8))
    ...
    def _load_template(self) -> None:
        result = self.controller.load_template(self.template_var.get())
        if result.error is not None:
            self._show_error(result.error)
            return
        if self.editor_text is not None:
            self._replace_editor_contents(self.controller.scenario_draft_text)
            self.editor_status_var.set(
                "Synthetic template loaded into the draft. It is not active until Validate and Use."
            )
        else:
            self._open_editor()
            self._replace_editor_contents(self.controller.scenario_draft_text)
        ...
        self.status_var.set("Template loaded into the draft. Nothing was written and the active scenario is unchanged.")

    def _save_template(self) -> None:
        path = self._filedialog.asksaveasfilename(
            parent=self.root,
            title="Save synthetic template",
            defaultextension=".json",
            filetypes=(("JSON files", "*.json"), ("All files", "*.*")),
        )
        if not path:
            return
        result = self.controller.write_template_file(self.template_var.get(), path)
        if result.error is not None:
            self._show_error(result.error)
            return
        self.status_var.set(f"Template saved to {path}")

    def _save_revision(self) -> None:
        folder = self._filedialog.askdirectory(parent=self.root, title="Select an existing folder for scenario revisions")
        if not folder:
            return
        result = self.controller.save_revision(folder)
        if result.error is not None:
            self._show_error(result.error)
            return
        saved = result.report.get("path", "") if result.report else ""
        self.status_var.set(f"Revision saved to {saved}")

    def _compare_scenarios(self) -> None:
        before = self._filedialog.askopenfilename(
            parent=self.root, title="Select the first scenario file", filetypes=(("JSON files", "*.json"), ("All files", "*.*"))
        )
        if not before:
            return
        after = self._filedialog.askopenfilename(
            parent=self.root, title="Select the second scenario file", filetypes=(("JSON files", "*.json"), ("All files", "*.*"))
        )
        if not after:
            return
        result = self.controller.scenario_diff(before, after)
        if result.error is not None:
            self._show_error(result.error)
            return
        self.format_var.set("markdown")
        self._refresh_preview()
        self.status_var.set("Scenario difference shown. Save Report writes it only after you choose a path.")
    ...
    def _show_error(self, error: str | None) -> None:
        ...
        self._messagebox.showerror("Corridor Lab", message, parent=self.root)
    ...
    def _open_editor(self) -> None:
        ...
        self._ttk.Button(controls, text="Load Fictional Template", command=self._load_editor_template).grid(row=0, column=0, padx=(0, 6))
        self._ttk.Button(controls, text="Validate and Use", command=self._validate_editor).grid(row=0, column=1, padx=6)
        self._ttk.Button(controls, text="Save Scenario As...", command=self._save_editor_scenario).grid(row=0, column=2, padx=6)
        self._ttk.Label(controls, textvariable=self.editor_status_var, wraplength=350).grid(row=0, column=3, sticky="w", padx=(12, 0))
        self.editor_text = self._scrolledtext.ScrolledText(editor, wrap="none", undo=True, font=("TkFixedFont", 10))
    ...
    def _choose_scenario(self) -> None:
        path = self._filedialog.askopenfilename(parent=self.root, title="Select fictional scenario JSON", filetypes=(("JSON files", "*.json"), ("All files", "*.*")))
        if not path:
            return
    ...
    def _choose_route_file(self) -> None:
        path = self._filedialog.askopenfilename(parent=self.root, title="Select fictional route JSON", filetypes=(("JSON files", "*.json"), ("All files", "*.*")))
        if path:
            self._select_routes(path)

    def _choose_route_folder(self) -> None:
        path = self._filedialog.askdirectory(parent=self.root, title="Select folder of fictional route JSON files")
        if path:
            self._select_routes(path)
...
def launch_gui() -> int:
    ...
    try:
        ...
        return _startup_failure()
    CorridorLabApp(root, ttk, scrolledtext, filedialog, messagebox, tk)
    root.mainloop()
    return 0
