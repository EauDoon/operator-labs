...
def _add_output_options(
    ...
    *,
    ...
    parser.add_argument("--evidence", help="also write a self-explaining evidence document to this JSON path")


def _write_evidence(args: argparse.Namespace, report: dict[str, object], argv: Sequence[str] | None) -> None:
    """Write the optional evidence bundle after a successful report emission."""
    from .evidence import build_evidence, write_evidence

    if not getattr(args, "evidence", None):
        return
    command = "corridorlab " + " ".join(argv) if argv is not None else args.command
    inputs: list[Path] = []
    scanned: list[Path] = []
    for name in ("scenario", "baseline", "routes", "input_dir"):
        ...
        for source_text in sources:
            ...
            (scanned if source.is_dir() else inputs).append(source)
    document = build_evidence(report, scenario_source=str(args.scenario) if getattr(args, "scenario", None) else "",
                              routes_source=str(args.routes) if getattr(args, "routes", None) else "",
                              notes="Generated by corridorlab " + (args.command or ""))
    write_evidence(args.evidence, document, inputs=tuple(inputs), scanned_dirs=tuple(scanned))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "batch":
            ...
            _write_evidence(args, batch_report, argv)
        ...
        if args.command == "robustness-review":
            ...
            _write_evidence(args, report, argv)
            return 0
