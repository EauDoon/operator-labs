...
Inside a project, `corridorlab project add-variant DIR --variant tight --changes "transaction.deadline_hours=1;route.ROUTE_ID.fixed_fee_send=2.00"` derives a named variant from the project scenario: only the declared transaction fields (send amount, deadline, volume) and declared route fields (rate, fees, spread, liquidity) change, and every other declared field is preserved. A variant may instead derive from another saved variant with `--base NAME`, building a chain materialized in dependency order; missing bases, self-reference, and cycles are rejected. `project show-variant DIR --variant tight` shows the cumulative assumption diff — exactly what changes, with declared base values — before any result is shown. `project compare-variants DIR [--variant ...]` presents route performance across the declared variant set (recipient outcomes, sender costs, deadline probability, tail hours, and guardrail satisfaction when an objective is declared) with currencies and units explicit; variants are declared cases, not forecasts, and no composite score is computed. `project run-variants DIR --experiment NAME [--variant ...]` reruns one saved experiment per variant. The desktop exposes the same actions in the Investigate tab, including **Save Variant...** with a Base selector for chaining, and applied variants stay in-memory, unsaved, and clearly labeled.

## Target and constraint analysis

`corridorlab target-search scenario.json --parameter send_amount --values 50,100,200 --constraint "expected_sender_cost_at_most=20" --constraint "probability_by_deadline_at_least=0.8"` evaluates every declared candidate against every declared constraint for each embedded route. Results name the smallest tested feasible value per route, list constraint failures with observed values and thresholds, mark invalid candidates honestly (fixed recovery amounts do not scale with the send amount, so small candidates can be structurally invalid), and mark routes whose tested set contains no feasible candidate as `unreachable_within_tested_set`. Supported constraints: `expected_sender_cost_at_most`, `expected_recipient_amount_at_least`, `probability_by_deadline_at_least`, `tail_completion_time_hours_at_most`. The search is exhaustive over the explicitly declared values only; the smallest tested feasible value is not a mathematical optimum.

`corridorlab robustness-review a.json b.json ... --constraint "..."` reviews constraint satisfaction across two or more supplied scenarios. Routes are reported per scenario with observed values, and each route's first failing scenario is named. Scenarios must share currencies, precisions, and rounding. Scenarios are declared cases, not forecasts, and no completeness claim is implied. Both commands support all tabular formats and protected `--output` paths. In the desktop Investigate tab, **Target Search** covers declared candidates and **Robustness Across Project Scenarios** uses the project's base scenario and derived variants as the declared case set.
