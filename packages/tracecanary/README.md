...
On POSIX shells, use `PYTHONPATH=src python -m tracecanary.gui`. From a checkout on Windows, double-click [TraceCanary.pyw](TraceCanary.pyw) instead. The desktop window is organized as seven keyboard-reachable tabs (`Ctrl+1` through `Ctrl+7`) around the questions an operator needs answered, with a shared result panel (human and JSON views plus `Save Report`, with find-in-report highlighting) underneath:

1. **Files and Starters** — selectors for the contract, sanitized input trace, baseline, candidate, a batch directory, and the unsanitized positive control; `Run Built-in Demo` (an in-memory safe check), `Create Synthetic Starter Files...`, which writes the fictional bundle into an empty directory you choose and prepares the selectors, plus **Open Project...** and **Save Project As...** for saved local projects.
2. **Is the Contract Usable?** — `Validate`, `Inspect Contract` (value-free check inventory, limits, and direct retention conflicts), `Review Contract` (conservative, actionable diagnostics), and `Edit Contract JSON...` (a transactional editor whose `Save Contract As...` is explicitly labeled as writing canary configuration).
...
6. **Batch Directories** — `Run Batch Check` over the selected directory (bounded enumeration, no symlinked files, per-file results, unresolved precedence, optional baseline comparison, opt-in paths) and `Run Coverage Batch` (aggregate sums with an optional exact per-file ratio gate), both in bounded background execution.
7. **Regression Campaign** — one bounded pass over the current selections with separate phase meanings (contract, control, baseline, candidates, batch, coverage thresholds); `Save Summary As...` for the value-free summary, `Save Evidence As...` for the value-free evidence document, and `Compare Saved Summaries...` with strict compatibility checks.
